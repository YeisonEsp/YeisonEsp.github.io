...
if (!isset($_GET["empretranit"]))           //VALIDAR QUE EL NIT DE LA EMPRESA ESTÉ DEFINIDO Y NO SEA NULO
{
    echo "<script>
            localStorage.clear();
            window.history.go(-1);
...
switch(true)
{
    case ($role=='Administrador') || ($role=='Vendedor'):           //VALIDAR QUE SE EJECUTE LO SIGUIENTE SOLO SI ES ADMINISTRADOR O VENDEDOR
        ...
        try{
            $sentencia = $base_de_datos->prepare("SELECT fun_active_empresatransporte( ?,?,? );");
            $sentencia->execute([$empretranit, $usua, $role]);                          //QUERY BY EXAMPLE
